...
def timer_js():
    return '''<script>function tick(){document.querySelectorAll('[data-start]').forEach(e=>{const s=Number(e.dataset.start);const sec=Math.max(0,Math.floor(Date.now()/1000-s));const h=Math.floor(sec/3600),m=Math.floor((sec%3600)/60),x=sec%60;e.textContent=String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(x).padStart(2,'0');});document.querySelectorAll('[data-price][data-start]').forEach(e=>{const s=Number(e.dataset.start),p=Number(e.dataset.price);const sec=Math.max(0,Math.floor(Date.now()/1000-s));e.textContent=new Intl.NumberFormat('uz-UZ').format(sec*p/3600).replaceAll(',', ' ')+' so\'m';});}tick();setInterval(tick,1000);</script>'''
